...
{
    /**
     * Class used to access the current Gradle version used by Unity Editor.
     * Attempts to parse information from Preferences/External Tools, but if that fails it
     * executes a --version command with the Gradle .jar from cli.
     * 
    ...
    internal static class Gradle
    {
        ...
        public static bool TryGetVersion(out Version gradleVersion, out string diagnosticMessage)
        {
            ...
            if (!JavaUtils.canExecute)
            {
                (gradleVersion, diagnosticMessage) = (null, "No Java executable found.");
                return false;
            }

            var (stdout, stderr, exitCode) = JavaUtils.Execute(pathToGradleLauncher, "--version");

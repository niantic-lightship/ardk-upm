...
namespace Niantic.Lightship.AR.PlatformAdapterManager
{
    internal class CpuTexturesSetter : AbstractTexturesSetter
    {
        ...
        public CpuTexturesSetter(_PlatformDataAcquirer dataAcquirer, _FrameData frameData, _ICTrace ctrace, UInt64 ctraceId) : base(dataAcquirer, frameData, ctrace, ctraceId)
        {
        ...
        {
            const string cTraceName = "PAM::JpegCPUConversion";
            _ctrace.TraceEventAsyncBegin0("SAL",cTraceName, _ctraceId);
            _ctrace.TraceEventAsyncStep0("SAL", cTraceName, _ctraceId, "TryGetCpuImage");
            if (_cpuImage.valid || _platformDataAcquirer.TryGetCpuImage(out _cpuImage))
            {

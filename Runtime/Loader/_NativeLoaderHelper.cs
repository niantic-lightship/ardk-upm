    ...
    internal class _NativeLoaderHelper
    {
        ...
        public bool Initialize(XRLoaderHelper loader, LightshipSettings settings, bool isLidarSupported, bool isTest)
        {
            ...
            if (settings.UseLightshipDepth && (!settings.PreferLidarIfAvailable || !isLidarSupported))
            {
                ...
                var settingsUser = (_ILightshipSettingsUser)loader.GetLoadedSubsystem<XROcclusionSubsystem>();
            ...
            if (settings.UseLightshipSemanticSegmentation)
            {
                ...
                var settingsUser = (_ILightshipSettingsUser)loader.GetLoadedSubsystem<XRSemanticsSubsystem>();
                settingsUser.SetLightshipSettings(settings);
            }
            ...
            {
                var lightshipLocationService = ((_ILightshipSettingsUser)Input.location);
                lightshipLocationService.SetLightshipSettings(settings);

                var lightshipCompass = ((_ILightshipSettingsUser)Input.compass);

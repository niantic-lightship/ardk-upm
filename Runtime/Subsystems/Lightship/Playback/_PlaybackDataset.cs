...
using System.Linq;
using Niantic.Lightship.AR.Subsystems.Lightship.Playback;
...
namespace Niantic.Lightship.AR.Playback
{
    ...
    {
        public _PlaybackDataset(string content, string datasetPath)
        {
            _datasetPath = datasetPath;
            JsonUtility.FromJsonOverwrite(content, this);

            if (Frames.Any(f => f.LocationInfo != null))
        ...
        public class FrameMetadata
        {
            ...
            public LocationInfo LocationInfo => location.Latitude == 0 ? null : location;

            //  Camera-to-world frame pose
            [SerializeField]
            private float[] pose4x4;

            public Matrix4x4 Pose => pose4x4.FromColumnMajorArray().FromOpenGLToUnity();

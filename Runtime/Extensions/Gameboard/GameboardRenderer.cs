...
namespace Niantic.Lightship.AR.Extensions.Gameboard
{
    ...
    public class GameboardRenderer : MonoBehaviour
    {
        ...
        void UpdateMesh()
        {
            //this will build a mesh for the gameboard nav tiles
            //you can copy and customise this if you would like to stylise the gamebaoard surface.
            var vertices = new List<Vector3>();
            var triangles = new List<int>();
            var vIndex = 0;

            float offset = 0.002f;
            var halfSize = _gameboardManager.gameboard.Settings.TileSize / 2.0f;
            foreach (var surface in _gameboardManager.gameboard.Surfaces)
            {
                foreach (var center in surface.Elements.Select
                         (node => Utils.TileToPosition(node.Coordinates, surface.Elevation,
                             _gameboardManager.gameboard.Settings.TileSize)))
                {
